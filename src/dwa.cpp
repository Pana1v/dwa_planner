...
void DWA_node::setGoal(double x, double y)
{
    goal_x = x;
    goal_y = y;
    goal_converged = false;
...
double DWA_node::get_convergence_time(double target_x, double target_y)
{
    setGoal(target_x, target_y);

    while(!goal_converged && rclcpp::ok())
    {
        rclcpp::spin_some(shared_from_this());
        std::this_thread::sleep_for(std::chrono::milliseconds(100));// 
    }
    while(!goal_converged && timing_started && rclcpp::ok())
    {
...
double DWA_node::getPathLength()
{
    return total_path_length;
}
void DWA_node::resetPath()
{
    path_history.clear();
    total_path_length = 0.0;
...
double DWA_node::get_current_x() const{
    return current_odom.pose.pose.position.x;
}
double DWA_node::get_current_y() const{
    return current_odom.pose.pose.position.y;
}

void DWA_node::timer_callback()
{
    if(timing_started && !path_history.empty())
    {
        double current_x = get_current_x();
        double current_y = get_current_y();

        double last_x = path_history.back().first;
        double last_y = path_history.back().second;
    ...
    if(timing_started)
    {
        path_history.push_back(make_pair(get_current_x(), get_current_y()));
    }
    if(goal_converged)
    {
