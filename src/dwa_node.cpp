...
{   public:
    ...
    // Laser scan subscription with callback
    ...
    // Initialize distributions after parameters are set
    ...
    // Creating URD
    // uniform_real_distribution<double> speed_dist(0,max_speed);
    // uniform_real_distribution<double> rotation_dist(-max_rotation_speed, max_rotation_speed);
    ...
    vector <pair<double, double>> motion_ranges(double speed, double rotation_speed, double time_step, const nav_msgs::msg::Odometry& odom_data)
    {
    ...
    pair<double, double> path_gen()
    {
        ...
        // vector<pair<double, double>> trajectory = motion_ranges(speed, rotation_speed, time_step, current_odom);
        return {speed, rotation_speed};
    }

    void visualize_trajectories(const vector<vector<pair<double, double>>>& all_trajectories, int best_index)
    {
        // First, clear all previous trajectory markers
        ...
        for(int i = 0; i < all_trajectories.size(); i++)
        {
            ...
            // Add lifetime for automatic clearing
            path_marker.lifetime = rclcpp::Duration::from_seconds(0.3); // Clear after 0.5 seconds
            
            // Color based on whether it's the best trajectory
            if(i == best_index) {
    ...
    double collision_avoidance(const vector<pair<double, double>>&trajectory, const sensor_msgs::msg::LaserScan& scan_data, const nav_msgs::msg::Odometry& current_odom, double tolerance = 0.2)
    {
        ...
        // Determine movement direction from first few trajectory points
    ...
    pair<double, double> dwa_control(const nav_msgs::msg::Odometry& odom_data, const sensor_msgs::msg::LaserScan& scan_data)
{
    ...
    for(int i=0;i<1000;i++)
    {
        ...
        double collision_factor = collision_avoidance(trajectory, scan_data, odom_data, tolerance);
